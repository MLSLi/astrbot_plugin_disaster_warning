    ...
    async def build(
        ...
        *,
        ...
        try:
            ...
            context = build_earthquake_card_context(event, options)

            # 来源特有 footer
            context["footer_items"] = build_earthquake_card_footer(
                event, context["footer_items"], source_id
            )
            ...
            template_name = message_format_config.get("earthquake_card_template", "Aurora")
            resources_dir = os.path.join(self.plugin_root, "resources")
            # 优先使用 source 专用模板，fallback 到通用地震卡片模板
            template_path = os.path.join(
                resources_dir, "card_templates", template_name, f"{source_id}.html"
            )
            ...
            if playwright_mode == "remote":
                ...
                leaflet_path = os.path.abspath(os.path.join(resources_dir, "card_templates", "leaflet.js"))
                leaflet_css_path = os.path.abspath(os.path.join(resources_dir, "card_templates", "leaflet.css"))
            ...
            with open(map_helper_path, encoding="utf-8") as hf:
                context["map_render_helper_js"] = hf.read()

            template = Template(template_content)
            html_content = template.render(**context)

            card_cache_key = cache_key_builder(event, message_format_config, display_timezone)

            async def render_card() -> str | None:
                report_num = event.report_num or 1
                image_filename = f"eq_card_{event.id}_{report_num}.png"
