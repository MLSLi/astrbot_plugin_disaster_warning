...
使用 `/地震列表查询` 命令时，插件可以生成精美的，仿 `JQuake` 风格的历史地震列表卡片，一次性直观展示最近发生的多次地震事件及其震级、烈度/震度分布。`/近期地震查询` 可按时间窗口汇集各机构发布或已复核的地震信息，合并同一地震事件的重复记录，并按各来源提供的震级值从高到低排列。

查询会按数据覆盖与产品适用范围选择用于展示的机构记录：中国大陆及港澳地区优先采用中国地震台网（CENC）信息，中国台湾地区及附近海域优先采用中国台湾地区气象部门（CWA）信息，日本及附近海域优先采用日本气象厅（JMA）信息，其他地区优先采用美国地质调查局（USGS）信息。首选机构没有对应记录时，再按区域适用顺序选择其他机构记录。
...
| `/近期地震查询 [小时] [数量] [格式]` | `/近期地震` | 汇集时间窗口内各机构发布或已复核的地震信息，合并重复记录后按来源震级值从高到低排列 |
...
# 格式：/近期地震查询 [小时] [数量] [格式]
# 别名：/近期地震 [小时] [数量] [格式]
# 默认：最近 24 小时、9 条、卡片格式

# 1. 查询最近 24 小时内的 9 条地震信息
/近期地震查询

# 2. 查询最近 72 小时内的 20 条地震信息
/近期地震查询 72 20 card

# 3. 使用别名查询最近一周的 10 条结果，并以文本显示
/近期地震 168 10 text
...
> `/近期地震查询`同样基于插件本地数据库，汇集中国地震台网（CENC）、中国台湾地区气象部门（CWA）、日本气象厅（JMA）和美国地质调查局（USGS）发布或已复核的地震信息，不包含地震预警（EEW）、Global Quake 或自动测定记录。不同机构提供的震级值可能采用不同标度，仅按来源数值从高到低排列，不表示严格的跨机构比较。时间窗口范围为 1-720 小时，显示数量范围为 1-50 条。
> `card` 格式返回少于 5 条信息时使用一张列表卡片；在支持合并消息的平台上，达到 5 条时每条信息生成一张独立卡片，并通过合并消息发送。其他平台继续使用单张列表卡片。
